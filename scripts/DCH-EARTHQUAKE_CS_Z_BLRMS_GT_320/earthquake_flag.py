...
parser.add_argument('-c', '--channel', type=str, default='H1:ISI-GND_STS_ITMY_Z_BLRMS_30M_100M.rms', help='channel name (optional)')
...
segment_file = 'H1_EARTHQUAKE_GT_%s_%s-%s.xml' % (thresh, args.start, abs(args.end-args.start))

# name the DQ flag
optic = channel[0].split('_')[2]
flag_name = 'H1:DCH-EQ_%s_GT_%s:1' % (optic, thresh)
...
data = get_timeseries_dict(channel, active, frametype='H1_M')
